...
export const ViewerErrorScreen: React.FC = () => {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950 font-mono text-slate-400 select-none p-6 text-center">
      ...
      <a href={ROUTES.MODELS} className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 font-bold text-white text-xs tracking-wide uppercase transition-all duration-300 rounded-xl hover:scale-105 active:scale-95 outline-none select-none">Back to Models</a>
    </div>
  );
};
