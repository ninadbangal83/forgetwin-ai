...
import { useMeasurementState } from '@/features/viewer/hooks/useMeasurementState';

export function MeasurementPanel() {
    const { measurements, handleClear } = useMeasurementState();
    ...
    return (
        <div className="absolute bottom-24 left-4 bg-slate-900/80 backdrop-blur-xl shadow-2xl border border-slate-800/80 rounded-2xl p-5 w-72 z-50 text-sm select-none">
            <div className="flex justify-between items-center mb-3 border-b border-slate-800/60 pb-2">
                ...
                <button onClick={handleClear} className="text-[10px] uppercase font-bold text-red-400 hover:text-red-300 hover:bg-red-950/40 border border-transparent hover:border-red-800/40 px-2 py-1 rounded-xl transition-all">Clear</button>
            </div>
