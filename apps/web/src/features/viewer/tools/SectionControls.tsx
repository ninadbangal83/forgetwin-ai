    ...
    return (
        <div className="absolute bottom-24 left-4 bg-slate-900/80 backdrop-blur-xl shadow-2xl border border-slate-800/80 rounded-2xl p-5 w-72 z-50 text-sm select-none">
            ...
            <label className="flex items-center gap-3 mb-5 cursor-pointer hover:bg-slate-800/40 p-2 rounded-xl -mx-2 transition-colors">
                <input type="checkbox" checked={enabled} onChange={() => dispatch(toggleClipping())} className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-indigo-500 focus:ring-indigo-500" />
            ...
            <div className={`space-y-4 transition-all duration-300 ${!enabled ? 'opacity-40 pointer-events-none filter grayscale' : ''}`}>
                <div>
                    ...
                    <input type="range" min="-100" max="100" step="0.5" value={planes.x} onChange={(e) => dispatch(setClippingPlanes({...planes, x: parseFloat(e.target.value)}))} className="w-full accent-red-500 bg-slate-800" />
                </div>
                <div>
                    ...
                    <input type="range" min="-100" max="100" step="0.5" value={planes.y} onChange={(e) => dispatch(setClippingPlanes({...planes, y: parseFloat(e.target.value)}))} className="w-full accent-teal-400 bg-slate-800" />
                </div>
                <div>
                    ...
                    <input type="range" min="-100" max="100" step="0.5" value={planes.z} onChange={(e) => dispatch(setClippingPlanes({...planes, z: parseFloat(e.target.value)}))} className="w-full accent-indigo-500 bg-slate-800" />
                </div>
                ...
                <label className="flex items-center gap-3 mt-5 cursor-pointer pt-3 border-t border-slate-800/60 hover:bg-slate-800/40 p-2 rounded-xl -mx-2 transition-colors">
                    <input type="checkbox" checked={invert} onChange={() => dispatch(toggleClippingInvert())} className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-indigo-500 focus:ring-indigo-500" />
